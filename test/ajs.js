...
hexo.extend.console.register('ajs', 'Ajs2js', function(args) {
  ...
  const { input_path, output_path } = {
    input_path: config.ajs.input_path || theme.ajs.input_path,
    output_path: config.ajs.output_path || theme.ajs.output_path,
  };
  ...
  async function main() {
    ...
    logger.info('Ajs Files loaded in %s', cyan(interval));
    try {
      const files = await fs.readdir(input_path);
      await Promise.all(files.map(file => handleFile(file, theme, config)));
      logger.info(content)
      const interval = prettyHrtime(process.hrtime(start));
      logger.info('%s Ajs Files loaded in %s', file_cout, cyan(interval));
      process.exit(1);
    } catch (err) {
      logger.error(err);
      process.exit(1);
